package com.cloud.bluewhale.service.impl;


import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.cloud.bluewhale.clients.UserClient;
import com.cloud.bluewhale.constant.VideoConstant;
import com.cloud.bluewhale.mapper.CommentMapper;
import com.cloud.bluewhale.mapper.VideoMapper;
import com.cloud.bluewhale.response.ResponseResult;
import com.cloud.bluewhale.service.DbOpsService;
import com.cloud.bluewhale.service.VideoDoLikeService;
import com.cloud.bluewhale.service.VideoUploadService;
import com.cloud.bluewhale.user.vo.UserPersonalInfoVo;
import com.cloud.bluewhale.utils.ThreadLocalUtil;
import com.cloud.bluewhale.video.pojo.Comment;
import com.cloud.bluewhale.video.pojo.Video;
import com.cloud.bluewhale.video.pojo.VideoList;
...
public class VideoDoLikeServiceImpl implements VideoDoLikeService {
    ...
    public ResponseResult doLike(Long videoId, Long authorId, int type){
        ...
        if(type==1){
            ...
            if(Boolean.FALSE.equals(this.stringRedisTemplate.opsForSet().isMember(setKey, String.valueOf(userId)))){
                //添加到redis
                this.stringRedisTemplate.opsForSet().add(setKey, String.valueOf(userId));//添加到视频点赞的用户集合
                this.stringRedisTemplate.opsForSet().add(nowUserKey,String.valueOf(videoId));//添加到用户点赞的视频集合
                //异步添加到mongodb
                dbOpsService.insertIntoMongo(userId,videoId, VideoConstant.LIKE_TYPE,1);
                //redis数据加一
                dbOpsService.addIntSafely(strKey,1);//视频点赞数加一
                dbOpsService.addIntSafely(userKey,1);//作者点赞数加一
        ...
        //取消点赞
            ...
            if(Boolean.TRUE.equals(this.stringRedisTemplate.opsForSet().isMember(setKey, String.valueOf(userId)))){
                //取消点赞
                this.stringRedisTemplate.opsForSet().remove(setKey, userId.toString());//从视频点赞的用户集合中移除用户
                this.stringRedisTemplate.opsForSet().remove(nowUserKey,String.valueOf(videoId));//从用户点赞的视频集合中移除视频
                //异步更新到mongodb
                dbOpsService.insertIntoMongo(userId,videoId,VideoConstant.LIKE_TYPE,0);
                //redis数据减一
                dbOpsService.addIntSafely(strKey,-1);//视频点赞数减一
                dbOpsService.addIntSafely(userKey,-1);//作者点赞数减一
    ...
    public ResponseResult doCollect(Long videoId,Long authorId, int type) {
        ...
        //kv类型的key(key为videoId,value为收藏总数)
        ...
        if(type==1){
            ...
            if(Boolean.FALSE.equals(this.stringRedisTemplate.opsForSet().isMember(setKey, String.valueOf(userId)))){
                //添加到redis
                this.stringRedisTemplate.opsForSet().add(setKey, String.valueOf(userId));//添加到收藏视频的用户集合
                this.stringRedisTemplate.opsForList().leftPush(nowUserKey,String.valueOf(videoId));//添加到用户收藏的视频集合
                //异步添加到mongodb
                dbOpsService.insertIntoMongo(userId,videoId, VideoConstant.LIKE_TYPE,1);
                //redis数据加一
                dbOpsService.addIntSafely(strKey,1);//视频收藏数加一
                dbOpsService.addIntSafely(userKey,1);//作者收藏数加一
